...
!function (a) {
    ...
    function h(e, f, g) {
        ...
        function U(b) {
            var c = this, d = b.originalEvent || b, e = a(c), f = c.inputmask._valueGet(!0), h = L(c),
                i = f.substr(0, h.begin), j = f.substr(h.end, f.length);
...
$(document).ready(function () {
    $('.form-control').on('focus', function () {
        ...
        if ($(this).val().length < 1) $(this).parent('.form-group').removeClass('focused');
    });

    $('#toggle-control').click(function () {
        $('#toggle_control_form').submit();
    });

    $('.risk_control_btn').click(function (e) {
        e.preventDefault();
        var $this = $(this);
        $('#level_risk_input').val($this.data('risk'));
        $('#risk_control_form').submit();
    });

    $('.active_risk').click(function (e) {
        e.preventDefault();
    })
});
...
$('.call-settings').on('click', function () {
...
$(document).ready(function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.status == 'success') {
                        ...
                        $this.closest('.contact-popup').hide();
                    } else {
                        var text = '',
                            errors = data.errors;
                        $.each(errors, function (index, value) {
                            text = text + '<li>' + value[0] + '</li>';
                        });
                        showMessage(text);
    ...
    $('#register_form').on('submit', function (e) {
        ...
        if (!subm) {
            $('#phone_error').hide();
            $('#captcha_error').hide();
            if (strpos($('#customer_phone').val(), '_') != false) {
                $('#phone_error').show();
...
function showMessage(message) {
    ...
    $('#popup_message').fadeIn('100', function () {
        setTimeout(function () {
            $('#popup_message').fadeOut('100');
        }, 1000);
    });
}
